...
form a basis for the vector space $M_(22)$ of $2 x 2$ matrices.
...
has only the trivial solution, where $0$ is the $2 x 2$ zero matrix; and to prove that the matrices span $M_(22)$ we must show that every $2 x 2$ matrix
...
**Note**: The simplest of all vector spaces is the zero vector space $V = {0}$. This space is finite‐dimensional because it is spanned by the vector $0$. However, it has no basis in the sense of Definition 1 because ${0}$ is not a linearly independent set (why?). However, we will find it useful to define the empty set $0$ to be a basis for this vector space.
